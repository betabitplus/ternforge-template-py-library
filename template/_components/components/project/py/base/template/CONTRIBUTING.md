...
[docs/[[[ package_name ]]]/verification/README.md](docs/[[[ package_name ]]]/verification/README.md)
...
```bash
uv run pytest tests/[[[ package_name ]]]
```

Run only hermetic tests:

```bash
uv run pytest tests/[[[ package_name ]]] -m hermetic
```
...
```bash
direnv exec . uv run python examples/[[[ package_name ]]]/<module>.py
```

Keep examples focused on imports from `[[[ package_name ]]]`. If an example
...
```bash
direnv exec . uv run python -m workbench.[[[ package_name ]]].<module>
```
