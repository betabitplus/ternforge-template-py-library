...
```bash
direnv exec . uv run pytest tests/[[[ package_name ]]] -m hermetic -q
direnv exec . uv run pytest tests/[[[ package_name ]]]/integration/test_config_lifecycle.py -q --no-cov
```
...
and reselect `[[[ project_name ]]] (.venv)`.
